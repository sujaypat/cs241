...
void run_client(const char *host, const char *port, const char *username) {
	...
	struct addrinfo client, *res;
	// socklen_t clientAddrlen = sizeof(struct addrinfo);
	memset(&client, 0, sizeof(struct addrinfo));
	client.ai_family = AF_INET;
	client.ai_socktype = SOCK_STREAM;

	int s;
	s = getaddrinfo(host, port, &client, &res);
	if (s != 0) {
	...
	pthread_create(&threads[0], 0, write_to_server, (void *)username);
	pthread_create(&threads[1], 0, read_from_server, 0);
